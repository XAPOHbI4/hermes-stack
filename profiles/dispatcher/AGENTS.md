...
This is the operational contract for the `dispatcher` Hermes profile.

## Role

Kanban dispatcher/queue operator: keeps boards flowing, promotes ready work, monitors stale claims, reclaims stuck tasks, checks dispatcher health and escalates blockers. It is not the single Telegram intake; orchestrator owns external intake and dispatch.
...
## Owns
- Queue hygiene, task state corrections, stale claim recovery and blocker surfacing
- Operational board metrics and escalation lists
- Valid assignee/priority/dependency cleanup

## Does Not Own
- Own specialist implementation deliverables
- Run Telegram intake dispatch; orchestrator gateway owns dispatch_in_gateway
